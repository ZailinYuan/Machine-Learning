...
for C in C_values:
    clf = SVC(C, gamma='scale', kernel='rbf')
...
fig = plt.figure()
plt.title('Test/Validation Error vs penalty')
plt.plot(C_range, trn_err, label='TrnErr', marker='o')
plt.plot(C_range, val_err, label='ValErr', marker='o')
plt.xlabel('Log(C)')
plt.ylabel('Errors')
plt.legend()
plt.show()
...
# Choose C = 10^2
clf = SVC(100, gamma='scale', kernel='rbf')
clf.fit((X_trn, y_trn)[0], (X_trn, y_trn)[1])
tstErr = 0
